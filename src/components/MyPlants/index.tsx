...
const MyPlants = () => {
  ...
  const { data } = useQuery({ queryKey: ['my-plants'], queryFn: getMyPlants });
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-16">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {data?.map(({ id, name, currentStatus, soilType, data: plantData }) => (
          <div
            key={id}
          ...
          >
                  ...
                  {plantData.map(
                    (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600">
                          <span className="font-medium">Nutrient Level:</span>{' '}
                          {nutrientLevel}
                        </p>
                        <p className="text-sm text-gray-600">
            ...
            <Button onClick={() => handleDetailsClick(id)} className="mt-auto">
              Details
            </Button>
          </div>
        ))}
